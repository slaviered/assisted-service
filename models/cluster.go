...
type Cluster struct {
	...
	// Required: true
	// Enum: [insufficient ready error preparing-for-installation pending-for-input installing finalizing installed adding-hosts cancelled installing-pending-user-action error-pending-collecting-logs]
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["insufficient","ready","error","preparing-for-installation","pending-for-input","installing","finalizing","installed","adding-hosts","cancelled","installing-pending-user-action","error-pending-collecting-logs"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
const (
	...
	// ClusterStatusErrorPendingCollectingLogs captures enum value "error-pending-collecting-logs"
	ClusterStatusErrorPendingCollectingLogs string = "error-pending-collecting-logs"
)
