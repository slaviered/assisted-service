...
var _ = Describe("cluster install", func() {
	...
	Context("install cluster cases", func() {
		...
		Context("report logs progress", func() {
			verifyLogProgress := func(c *models.Cluster, host_progress string, cluster_progress string) {
			...
			It("log progress installation succeed", func() {
				...
				c := installCluster(clusterID)
				requested := string(models.LogsStateRequested)
				completed := string(models.LogsStateCompleted)
				for _, host := range c.Hosts {
					updateLogProgress(clusterID, host.ID, string(models.LogsTypeHost), requested)
				}
				updateLogProgress(clusterID, nil, string(models.LogsTypeController), requested)
				...
				for _, host := range c.Hosts {
					updateLogProgress(clusterID, host.ID, string(models.LogsTypeHost), completed)
					updateProgress(*host.ID, clusterID, models.HostStageDone)
				}
				waitForClusterState(ctx, clusterID, models.ClusterStatusFinalizing, defaultWaitForClusterStateTimeout, clusterFinalizingStateInfo)
				updateLogProgress(clusterID, nil, string(models.LogsTypeController), requested)
				...
				Expect(err).NotTo(HaveOccurred())
				waitForClusterState(ctx, clusterID, models.ClusterStatusInstalled, defaultWaitForClusterStateTimeout, "installed")
				updateLogProgress(clusterID, nil, string(models.LogsTypeController), completed)
				c = getCluster(clusterID)
